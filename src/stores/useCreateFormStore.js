...
export const useCreateFormStore = createWithEqualityFn((set) => {
  const updateConfig = (source, value) => set((state) => ({
    ...state,
    config: { ...state.config, [source]: value },
  }));

  const updateQuestion = (questionId, source, value) => set((state) => ({
    ...state,
  ...
  const getQuestion = (id) => {
    const question = useCreateFormStore.getState().questions.find((question) => question.id === id);
    if (!question) throw new Error('Question with id = ' + id + ' was not found');
    return question;
  };

  const addQuestion = (newQuestion) => set((state) => ({
    ...state,
    questions: [...state.questions, newQuestion],
  }));

  const duplicateQuestion = (id) => useCreateFormStore.getState()
    .addQuestion({
      ...useCreateFormStore.getState().getQuestion(id),
      id: uuid(),
    });

  const deleteQuestion = (id) => set((state) => ({
    ...state,
